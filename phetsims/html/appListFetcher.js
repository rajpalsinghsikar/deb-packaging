...
page.open('http://phet.colorado.edu/en/simulations', function (status) {
    ...
    var allAppsName = page.evaluate(function() {
        ...
        for(var i = 0; i<all.length;i++){
            if((all[i].innerHTML).indexOf('<span class="sim-display-badge sim-badge-html"></span>')>-1){
                var selectedApps = all[i].getElementsByClassName('simulation-link')[0];
                javaApps.push(selectedApps.href.replace('http://phet.colorado.edu/en/simulation/html/','') +"\n")
            }
        }
        return javaApps;
    });
